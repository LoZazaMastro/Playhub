...
try {
    ...
    & dotnet publish $appProject -c Release -r win-x64 --self-contained true --no-restore -p:Platform=x64 -p:WindowsAppSDKSelfContained=true -p:PlayhubUiReview=false -p:PlayhubUpdatePreview=false -o $publish
    if ($LASTEXITCODE) { throw 'App normal build failed.' }
    Assert-PreviewFlag (Join-Path $publish 'Playhub.dll') $false
