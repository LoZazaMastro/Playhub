...
function dashboardDocuments(): Document[] {
  ...
  const addDocument = (candidate: Document | null | undefined) => {
    try {
      if (candidate?.documentElement && !documents.includes(candidate)) documents.push(candidate);
    } catch {}
  };
  const addWindowDocument = (candidate: any) => {
    ...
    try { addDocument(candidate.m_BrowserWindow?.document); } catch {}
  ...
  if (Array.isArray(store?.OverlayWindows)) store.OverlayWindows.forEach(addWindowDocument);
  return documents;
}
...
export function markDashboardChrome(): void {
  dashboardDocuments().forEach((targetDocument) => {
    try {
      ...
      let style = targetDocument.getElementById(DASHBOARD_CHROME_STYLE_ID) as HTMLStyleElement | null;
      if (!style) {
...
export function clearDashboardChrome(): void {
  dashboardDocuments().forEach((targetDocument) => {
    try {
...
function dashboardRoot(): HTMLElement | null {
  for (const targetDocument of dashboardDocuments()) {
    try {
      const root = targetDocument.querySelector(".ph-dashboard") as HTMLElement | null;
      if (root) return root;
    } catch {}
  }
  return null;
...
function focusDashboard(selector: string): boolean {
  const target = dashboardRoot()?.querySelector(selector) as HTMLElement | null;
  if (!target) return false;
  try { target.focus({ preventScroll: true }); return target.ownerDocument.activeElement === target; }
  catch { return false; }
}
...
function activateDashboardSteamContext(): boolean {
  ...
  for (const steamWindow of candidates) {
    if (!steamWindow || visited.has(steamWindow)) continue;
...
function ensureDashboardFocus(): boolean {
  activateDashboardSteamContext();
  const dashboard = dashboardRoot();
  if (!dashboard) return false;
  if (dashboard.contains(dashboard.ownerDocument.activeElement)) return true;
...
export function focusDashboardSurface(): boolean {
  return ensureDashboardFocus();
}

function currentDashboardFocus(): HTMLElement | null {
  const root = dashboardRoot();
  const active = root?.ownerDocument.activeElement as HTMLElement | null;
  if (!root || !active || !root.contains(active)) return null;
  return active.closest<HTMLElement>("[data-ph-focusable='true']");
}

type TabId = "switcher" | "apps" | "system";
type IconType = (props: any) => any;
...
function sound(kind: "move" | "open") {
  try {
    const Context = (window as any).AudioContext || (window as any).webkitAudioContext;
    if (!Context) return;
    const context: AudioContext = audioContext ?? new Context();
    ...
    first.start(now); second.start(now + 0.008);
    first.stop(now + 0.2); second.stop(now + 0.2);
  } catch {}
}

function stopEvent(event: any, prevent = false) {
  try { if (prevent) event?.preventDefault?.(); event?.stopPropagation?.(); } catch {}
}

function FocusItem({ className = "", children, onPress, onFocus, ...props }: any) {
  const lastActivation = useRef(0);
  const activate = (event: any) => {
    stopEvent(event, true);
    const now = performance.now();
    if (now - lastActivation.current < 160) return;
  ...
  return (
    <Focusable
      {...props}
      tabIndex={0}
      data-ph-focusable="true"
      noFocusRing
      focusClassName="ph-focus"
      className={className}
      onActivate={activate}
      onClick={(event: any) => { if (event?.detail > 0) activate(event); }}
      onFocus={(event: any) => { sound("move"); onFocus?.(event); }}
    >
      {children}
    </Focusable>
  );
...
function gridDirectionFromKey(key: string): GridDirection | null {
...
function gridDirectionFromGamepad(button: unknown): GridDirection | null {
  if (button === GamepadButton?.DIR_LEFT) return "left";
  if (button === GamepadButton?.DIR_RIGHT) return "right";
  if (button === GamepadButton?.DIR_UP) return "up";
  if (button === GamepadButton?.DIR_DOWN) return "down";
  return null;
}
...
function moveGridFocus(event: any, direction: GridDirection | null) {
  ...
  for (const element of Array.from(grid.querySelectorAll<HTMLElement>("[data-ph-grid-index]"))) {
    ...
    if (direction === "left" && dx < -2 && sameRow) score = Math.abs(dx) + Math.abs(dy) * 5;
    if (direction === "right" && dx > 2 && sameRow) score = Math.abs(dx) + Math.abs(dy) * 5;
    if (direction === "up" && dy < -2 && sameColumn) score = Math.abs(dy) + Math.abs(dx) * 5;
    if (direction === "down" && dy > 2 && sameColumn) score = Math.abs(dy) + Math.abs(dx) * 5;
...
function focusSystemMetric(position = 0): boolean {
  const metrics = Array.from(dashboardRoot()?.querySelectorAll<HTMLElement>(".ph-history-card") ?? []);
...
function FocusGrid({ className = "", children, ...props }: any) {
  return (
    <Focusable
      {...props}
      className={className}
      flow-children="grid"
      data-ph-focus-grid="true"
    >
      {children}
    </Focusable>
  );
...
function Header({ tab, setTab, environment, logo }: { tab: TabId; setTab: (tab: TabId) => void; environment: DashboardEnvironment | null; logo: string }) {
  ...
  const tabs = useMemo(() => {
    ...
    ] as Array<{ id: TabId; label: string; icon: IconType }>;
  }, [copy]);
  return (
    <header className="ph-header">
      <div className="ph-brand">
        {logo ? <img src={logo} alt="Playhub" /> : <div className="ph-brand-fallback">playhub</div>}
      </div>
      <Focusable className="ph-tabs" flow-children="horizontal">
        {tabs.map((item) => {
          const Icon = item.icon;
          return (
            <FocusItem
              key={item.id}
              className={`ph-tab ${tab === item.id ? "ph-active" : ""}`}
              onPress={() => setTab(item.id)}
              onButtonDown={(event: any) => {
                if (item.id !== "system" || tab !== "system" || Number(event?.detail?.button) !== GamepadButton?.DIR_DOWN) return;
                stopDirectionalEvent(event);
                focusSystemMetric(0);
              }}
              onKeyDown={(event: any) => {
                if (item.id !== "system" || tab !== "system" || gridDirectionFromKey(event.key) !== "down") return;
                stopDirectionalEvent(event);
                focusSystemMetric(0);
              }}
              onOKActionDescription={item.label}
            >
...
function AppIcon({ source, fallback: Fallback = FiGrid }: { source?: string; fallback?: IconType }) {
...
function WindowCard({ entry, artwork, index, copy, onSelect, onAskClose }: {
  ...
  return (
    <FocusItem
      className="ph-window-card"
      data-ph-grid-index={index}
      data-window-card-handle={entry.handle}
      data-window-primary={entry.primary ? "true" : "false"}
      ...
      onSecondaryButton={(event: any) => { stopEvent(event, true); onAskClose(); }}
      onOKActionDescription={copy.open}
      onSecondaryActionDescription={copy.close}
      onFocus={(event: any) => {
    ...
    >
      <div className="ph-window-title">
        {icon ? <img className="ph-app-icon" src={icon} alt="" /> : isSteam ? <SiSteam /> : <FiMonitor />}
        <span>{entry.title}</span>
      </div>
      <div className="ph-window-frame" data-window-handle={entry.handle}>
        {artwork ? <img src={artwork} alt="" /> : (
          <div className="ph-window-placeholder">
            {icon ? <img src={icon} alt="" /> : <FiMonitor />}
          </div>
        )}
        {entry.minimized ? <div className="ph-window-meta">{copy.minimized}</div> : null}
      </div>
    </FocusItem>
  );
}

async function loadFallback(entry: WindowEntry): Promise<string> {
...
function TaskSwitcher({ copy, onReady, onSelectWindow }: {
  copy: Copy;
  onReady: () => void;
  onSelectWindow: (entry: WindowEntry) => void;
}) {
  ...
  useLayoutEffect(() => {
    if (!primaryHandle || focusedPrimaryHandle.current === primaryHandle) return;
    ...
    const run = async () => {
      for (let index = 0; index < candidates.length && alive; index += 2) {
        ...
        await Promise.all(pair.map(async (entry) => {
          ...
          if (!alive) return;
          if (fallback) setArtwork((current) => {
            if (current[entry.handle] === fallback) return current;
  ...
  return (
    <div className="ph-page ph-switcher-page">
      {windows.length === 0 ? (
        ...
        <Focusable ref={rail} className={`ph-window-rail ${orderedWindows.length === 1 ? "ph-single" : ""}`} flow-children="horizontal" data-ph-focus-grid="true">
          {orderedWindows.map((entry, index) => (
            <WindowCard
              key={entry.handle}
              entry={entry}
              artwork={artwork[entry.handle] ?? ""}
              index={index}
              copy={copy}
              onSelect={() => onSelectWindow(entry)}
              onAskClose={() => void closeEntry(entry, index)}
            />
          ))}
        </Focusable>
      )}
...
function AppsTab({ copy, extra, onOptions, onLaunch, library, setLibrary }: {
  copy: Copy;
  extra: ExtraCopy;
  onOptions: (shortcut: ShortcutEntry, refresh: () => Promise<void>) => void;
  onLaunch: (shortcut: ShortcutEntry) => void;
  library: boolean;
  setLibrary: (value: boolean) => void;
}) {
          ...
          <FocusGrid className="ph-grid">
            {programs.map((program, index) => (
              <AppTile
                ...
                title={program.name}
                icon={iconSource(program.iconBase64)}
                copy={copy}
                index={index}
  ...
  return (
    <div className="ph-page ph-page-scroll">
      <FocusGrid className="ph-grid">
        <AppTile title={copy.addApp} fallback={FiPlus} copy={copy} extra={extra} onPress={() => void openLibrary()} />
        {shortcuts.map((shortcut, index) => (
          <AppTile
            key={shortcut.id}
            title={shortcut.name}
            icon={iconSource(shortcut.iconBase64)}
            copy={copy}
            extra={extra}
            index={index + 1}
...
function HistoryMetric({ title, value, detail, series, fixedPeak, index }: { title: string; value: string; detail: string; series: number[]; fixedPeak?: number; index: number }) {
  const lineRef = useRef<SVGPolylineElement | null>(null);
  const samplesRef = useRef<Array<{ value: number; at: number }>>([]);
  ...
  useEffect(() => {
    ...
    targetRef.current = next;
    if (samplesRef.current.length > 32) samplesRef.current.splice(0, samplesRef.current.length - 32);
  }, [series]);
  useEffect(() => {
  ...
  return (
    <FocusItem
      ...
      onButtonDown={(event: any) => {
        ...
        if (direction === "down") { stopDirectionalEvent(event); focusDashboard(".ph-restart-decky"); return; }
        if (direction === "up") { stopDirectionalEvent(event); focusDashboard(".ph-tab.ph-active"); return; }
    ...
    >
      <div className="ph-history-label">{title}</div>
      <div className="ph-history-value">{value}</div>
      <svg className="ph-history-chart" viewBox="0 0 184 40" preserveAspectRatio="none" aria-hidden="true"><polyline ref={lineRef} points="" /></svg>
      <div className="ph-history-detail">{detail}</div>
    </FocusItem>
  );
}

function SystemTab({ copy, extra, onConfirm }: { copy: Copy; extra: ExtraCopy; onConfirm: (title: string, name: string, action: () => void) => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setProcesses((current) => {
        if (current.length === 0) return nextProcesses;
    ...
    <div className="ph-page ph-page-scroll">
      <div className="ph-system-stack">
        <FocusGrid className="ph-metrics-rail" onWheel={(event: any) => { if (Math.abs(event.deltaY) > Math.abs(event.deltaX)) event.currentTarget.scrollLeft += event.deltaY; }}>
          ...
          <HistoryMetric index={1} title={copy.gpu} value={usage?.gpuAvailable ? `${Math.round(usage.gpuPercent)}%` : "--"} detail={usage?.gpuAvailable ? "GPU" : "--"} series={history.gpu ?? []} fixedPeak={100} />
          <HistoryMetric index={2} title={copy.memory} value={`${Math.round(usage?.memoryPercent ?? 0)}%`} detail={`${((usage?.memoryUsedMb ?? 0) / 1024).toFixed(1)} / ${((usage?.memoryTotalMb ?? 0) / 1024).toFixed(1)} GB`} series={history.memory ?? []} fixedPeak={100} />
          <HistoryMetric index={3} title={extra.network} value={formatRate(usage?.networkBytesPerSecond ?? 0)} detail={extra.network} series={history.network ?? []} />
          {(usage?.disks ?? []).map((disk, diskIndex) => <HistoryMetric index={4 + diskIndex} key={disk.name} title={`${extra.disk} ${disk.name}`} value={formatRate(disk.bytesPerSecond)} detail={disk.kind ? disk.kind.toUpperCase() : extra.disk} series={history[`disk:${disk.name}`] ?? []} />)}
        </FocusGrid>
        <div className="ph-system-lower">
        <div className="ph-tile ph-process-panel">
          <div className="ph-process-heading">
            ...
            <FocusItem
              ...
              onButtonDown={(event: any) => {
                ...
                if (direction === "down") { stopDirectionalEvent(event); focusDashboard(".ph-process-row"); }
              }}
              onKeyDown={(event: any) => {
                const direction = gridDirectionFromKey(event.key);
                if (direction === "up") { stopDirectionalEvent(event); focusSystemMetric(0); }
            ...
            ><FiRefreshCw /><span>{copy.restartDecky}</span></FocusItem>
          </div>
          <Focusable className="ph-process-list" flow-children="vertical">
            {processes.slice(0, 18).map((process) => (
              <FocusItem
                key={process.id}
                className="ph-process-row"
                onPress={() => setSelectedProcess(process)}
                onOKActionDescription={copy.open}
                onButtonDown={(event: any) => {
                  const direction = gridDirectionFromGamepad(event?.detail?.button);
                  if (direction === "up" && process === processes[0]) { stopDirectionalEvent(event); focusDashboard(".ph-restart-decky"); }
                  if (direction === "right") { stopDirectionalEvent(event); focusDashboard(".ph-restart-decky"); }
                }}
                onKeyDown={(event: any) => {
                  const direction = gridDirectionFromKey(event.key);
                  if (direction === "up" && process === processes[0]) { stopDirectionalEvent(event); focusDashboard(".ph-restart-decky"); }
                ...
                onFocus={(event: any) => {
                  const item = event?.currentTarget as HTMLElement | null;
                  const container = item?.closest?.(".ph-process-list") as HTMLElement | null;
                  if (!item || !container) return;
              ...
              >
                <div className="ph-process-name">{process.name}</div><div className="ph-process-stat">{process.cpuPercent}%</div><div className="ph-process-stat">{Math.round(process.memoryMb)} MB</div>
              </FocusItem>
            ))}
          </Focusable>
        </div>
...
function ProcessOptionsSheet({ process, copy, onCancel, onClose, onKill }: {
  ...
  return (
    <Focusable className="ph-confirm-backdrop ph-process-options" onCancel={(event: any) => { stopEvent(event, true); onCancel(); }} onCancelButton={(event: any) => { stopEvent(event, true); onCancel(); }}>
      <div className="ph-confirm">
        <div className="ph-confirm-title">{process.name}</div>
        <div className="ph-confirm-name">PID {process.id}</div>
        <Focusable className="ph-confirm-actions ph-three" flow-children="horizontal">
          <FocusItem preferredFocus className="ph-confirm-button" onPress={onCancel}>{copy.cancel}</FocusItem>
          <FocusItem className="ph-confirm-button" onPress={onClose}>{copy.close}</FocusItem>
          <FocusItem className="ph-confirm-button ph-danger" onPress={onKill}>{copy.forceClose}</FocusItem>
        </Focusable>
      </div>
    </Focusable>
  );
...
function ConfirmSheet({ title, name, copy, onCancel, onConfirm }: { title: string; name: string; copy: Copy; onCancel: () => void; onConfirm: () => void }) {
  useEffect(() => {
    const timer = window.setTimeout(() => focusDashboard(".ph-confirm-button"), 40);
    return () => window.clearTimeout(timer);
  }, []);
  return (
    <Focusable className="ph-confirm-backdrop" onCancel={(event: any) => { stopEvent(event, true); onCancel(); }} onCancelButton={(event: any) => { stopEvent(event, true); onCancel(); }}>
      <div className="ph-confirm">
        <div className="ph-confirm-title">{title}</div><div className="ph-confirm-name">{name}</div>
        <Focusable className="ph-confirm-actions" flow-children="horizontal">
          <FocusItem preferredFocus className="ph-confirm-button" onPress={onCancel}>{copy.cancel}</FocusItem>
          <FocusItem className="ph-confirm-button ph-danger" onPress={onConfirm}>{copy.continue}</FocusItem>
        </Focusable>
      </div>
    </Focusable>
  );
...
function AppOptionsSheet({ shortcut, copy, extra, onCancel, onRemove, onSave }: {
  ...
  return (
    <Focusable className="ph-confirm-backdrop" onCancel={(event: any) => { stopEvent(event, true); onCancel(); }} onCancelButton={(event: any) => { stopEvent(event, true); onCancel(); }}>
      <div className="ph-confirm">
        <div className="ph-confirm-title">{extra.options}</div>
        <div className="ph-confirm-name">{shortcut.name}</div>
...
function DashboardSurface() {
  const [environment, setEnvironment] = useState<DashboardEnvironment | null>(null);
  const [logo, setLogo] = useState("");
  const [tab, setTab] = useState<TabId>("switcher");
  const [appsLibrary, setAppsLibrary] = useState(false);
  ...
  useEffect(() => {
    ...
    void readEnvironment().then(async (value) => {
      ...
      if (value?.logoPath) setLogo(await loadImage(value.logoPath));
    });
    return () => { alive = false; };
  }, []);
  ...
  const rememberModalFocus = useCallback(() => {
    ...
    if (active && !active.closest(".ph-confirm-backdrop")) modalReturnFocus.current = active;
  }, []);
  ...
  const ask = (title: string, name: string, action: () => void) => {
    rememberModalFocus();
  ...
  const selectWindow = useCallback((entry: WindowEntry) => {
    const isSteam = /^(steam|steamwebhelper)$/i.test(entry.processName);
    leaveDashboard(isSteam ? "steam" : "window", entry.handle);
  }, [leaveDashboard]);
  const launchShortcut = useCallback((shortcut: ShortcutEntry) => {
    leaveDashboard("app", shortcut.id);
  }, [leaveDashboard]);
  const cancelDashboard = useCallback((event?: any) => {
    const now = performance.now();
    ...
    if (tab !== "switcher") { changeTab("switcher"); return true; }
    closeDashboard();
    return true;
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (direction && moveGridFocus(event, direction)) return;
      if (event.key === "Escape") {
        event.preventDefault();
        event.stopPropagation();
    ...
    const targets = dashboardDocuments().map((targetDocument) => targetDocument.defaultView).filter(Boolean) as Window[];
  ...
  return (
    <Focusable
      ...
      onButtonDown={(event: any) => {
        ...
        const processRow = (event?.target as HTMLElement | null)?.closest?.(".ph-process-row");
      ...
      onCancelButton={cancelDashboard}
    ...
    >
      ...
      {confirm ? (
        <ConfirmSheet
          ...
          onConfirm={() => { const action = confirm.action; dismissConfirm(); action(); window.setTimeout(ensureDashboardFocus, 520); }}
        />
      ) : null}
      {appOptions ? (
        <AppOptionsSheet
          shortcut={appOptions.shortcut}
          copy={copy}
          extra={extra}
          onCancel={dismissAppOptions}
          onRemove={() => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (targetDocument?.body) {
        let target = targetDocument.getElementById("ph-dashboard-overlay-root") as HTMLElement | null;
        if (!target) {
