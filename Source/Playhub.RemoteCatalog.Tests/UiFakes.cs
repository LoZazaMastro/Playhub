...
namespace Playhub
{
    public sealed partial class MainWindow
    {
        ...
        internal static async Task RunCatalogUiTests()
        {
            ...
            { Content = new ByteArrayContent(IntegrationTests.Serialize(new RemotePluginCatalog { CatalogRevision = 6, Plugins = new[] { extra } })) });
            window._remotePluginCatalog = fixture.Service;
#endif
            AppPaths.Reads = 0;
            await window.RefreshPluginsAsync();
            IntegrationTests.Check(window._renders == 1 && window._plugins.Count == 196 && window._errors == 0, "Local baseline did not render first.");
#if PLAYHUB_UI_REVIEW
            ...
            IntegrationTests.Check(window._renders == 2 && window._plugins.Count == 197 && window._visiblePluginCatalogRevision == 6,
            ...
            fixture.Handler.Respond = async (_, token) =>
            {
                ...
                { Content = new ByteArrayContent(IntegrationTests.Serialize(new RemotePluginCatalog { CatalogRevision = 7, Plugins = new[] { extra with { Version = "3.0.0" } } })) };
            };
            window.QueueRemotePluginCatalogRefresh();
            window.DispatcherQueue.RunNext();
            await entered.Task;
            ...
            await window.RefreshPluginsAsync();
            IntegrationTests.Check(window._visiblePluginCatalogRevision == 7 && window._renders == 3, "Deferred revision not applied on natural refresh.");
            window.DispatcherQueue.RunNext();
            await WaitFor(() => !window._remotePluginCatalogRefreshQueued);
            ...
            window.DispatcherQueue.RunNext();
            await WaitFor(() => !window._remotePluginCatalogRefreshQueued);
            IntegrationTests.Check(window._renders == 4 && window._visiblePluginCatalogRevision == 7,
