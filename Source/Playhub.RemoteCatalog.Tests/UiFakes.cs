...
namespace Playhub
{
    public sealed partial class MainWindow
    {
        ...
        internal static async Task RunCatalogUiTests()
        {
            ...
            { Content = new ByteArrayContent(IntegrationTests.Serialize(new RemotePluginCatalog { CatalogRevision = 2, Plugins = new[] { extra } })) });
            window._remotePluginCatalog = fixture.Service;
#endif
            AppPaths.Reads = 0;
            await window.RefreshPluginsAsync();
            IntegrationTests.Check(window._renders == 1 && window._plugins.Count == 173 && window._errors == 0, "Local baseline did not render first.");
#if PLAYHUB_UI_REVIEW
            ...
            IntegrationTests.Check(window._renders == 2 && window._plugins.Count == 174 && window._visiblePluginCatalogRevision == 2,
            ...
            fixture.Handler.Respond = async (_, token) =>
            {
                ...
                { Content = new ByteArrayContent(IntegrationTests.Serialize(new RemotePluginCatalog { CatalogRevision = 3, Plugins = new[] { extra with { Version = "3.0.0" } } })) };
            };
            window.QueueRemotePluginCatalogRefresh();
            window.DispatcherQueue.RunNext();
            await entered.Task;
