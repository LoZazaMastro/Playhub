...
public sealed partial class PluginCatalogService
{
    ...
    private static readonly IReadOnlyDictionary<string, string> PlayhubKeywords =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["Shortcuts"] = "shortcuts tabs QAM quick access menu plugins icons Decky catalog-added:2026-09-04",
...
• Stato HDR letto direttamente da Windows (DisplayConfig / Advanced Color) invece di affidarsi a uno stato salvato dal plugin."),
        new PluginDefinition(
            ...
            "Arrange Steam, Decky and your favourite plugin tabs from one place.",
            @"Shortcuts gives you one controller-friendly place to arrange Steam's native Quick Access Menu tabs, Decky and compatible plugin shortcuts. Changes appear immediately on the Steam Home screen and while playing.

## What it does
• Reorders Steam's native QAM tabs, Decky and compatible custom tabs.
• Turns loaded Decky plugin panels into independent shortcuts.
• Keeps every plugin's original Decky entry available.
• Lets you use the original icon or choose from the included Tabler icons.
• Saves icon and tab choices across Steam and Shortcuts restarts.
• Restores temporarily unavailable plugins when they return.

## Note
• It uses guarded integration with Steam and Decky's internal QAM tab registry because no public API currently covers independent top-level tabs and their ordering. A future Steam or Decky update may require an adjustment."),
        new PluginDefinition(
...
• Richiede l'app ufficiale Proton VPN per Windows già installata e configurata.")
    ...
    private static bool IsBlockedPluginIdentity(params string[] identities)
    {
        foreach (var identity in identities.Where(value => !string.IsNullOrWhiteSpace(value)))
        {
            ...
            if (normalized is "varta" or "vartaplugin" or "gamingmode" or "playhubgamingmode" or "quicksettings" or "shortcuts")
            {
                return true;
            }
            ...
            if (!string.IsNullOrWhiteSpace(repository) &&
                Normalize(repository[(repository.IndexOf('/') + 1)..]) is "varta" or "vartaplugin" or "gamingmode" or "playhubgamingmode" or "quicksettings" or "shortcuts")
            {
                return true;
            }
        }
...
• Состояние HDR читается напрямую из Windows (DisplayConfig / Advanced Color), а не из сохранённого состояния плагина.")
            ...
            {
                ["it"] = new PluginText(
                    "Organizza da un unico posto le schede di Steam, Decky e dei tuoi plugin preferiti.",
                    @"Shortcuts riunisce in un'unica schermata pensata per il controller le schede native del menu rapido di Steam, Decky e le scorciatoie dei plugin compatibili. Le modifiche vengono applicate subito nella Home di Steam e mentre giochi.

## Cosa fa
• Riordina le schede native del QAM di Steam, Decky e le schede personalizzate compatibili.
• Trasforma i pannelli dei plugin Decky caricati in scorciatoie indipendenti.
• Mantiene disponibile la voce originale di ogni plugin dentro Decky.
• Permette di scegliere l'icona originale o una delle icone Tabler incluse.
• Conserva icone e ordine delle schede dopo il riavvio di Steam o Shortcuts.
• Ripristina i plugin temporaneamente non disponibili quando tornano attivi.

## Nota
• Usa un'integrazione protetta con Steam e il registro interno delle schede QAM di Decky, perché al momento non esiste un'API pubblica che gestisca schede principali indipendenti e il loro ordine. Un futuro aggiornamento di Steam o Decky potrebbe richiedere un adeguamento."),
                ["en"] = new PluginText(
                    "Arrange Steam, Decky and your favourite plugin tabs from one place.",
                    @"Shortcuts gives you one controller-friendly place to arrange Steam's native Quick Access Menu tabs, Decky and compatible plugin shortcuts. Changes appear immediately on the Steam Home screen and while playing.
...
• It uses guarded integration with Steam and Decky's internal QAM tab registry because no public API currently covers independent top-level tabs and their ordering. A future Steam or Decky update may require an adjustment."),
                ["es"] = new PluginText(
                    "Organiza desde un solo lugar las pestañas de Steam, Decky y tus plugins favoritos.",
                    @"Shortcuts reúne en un único espacio pensado para el mando las pestañas nativas del menú de acceso rápido de Steam, Decky y los accesos directos de plugins compatibles. Los cambios aparecen al instante tanto en el inicio de Steam como mientras juegas.

## Qué hace
• Reordena las pestañas QAM nativas de Steam, Decky y las pestañas personalizadas compatibles.
• Convierte los paneles de plugins de Decky cargados en accesos directos independientes.
• Mantiene disponible la entrada original de cada plugin dentro de Decky.
• Permite usar el icono original o elegir entre los iconos de Tabler incluidos.
• Conserva la elección de iconos y el orden de las pestañas tras reiniciar Steam o Shortcuts.
• Restaura los plugins que no estaban disponibles temporalmente cuando vuelven.

## Nota
• Usa una integración protegida con Steam y el registro interno de pestañas QAM de Decky porque aún no existe una API pública para gestionar pestañas principales independientes y su orden. Una futura actualización de Steam o Decky podría requerir adaptar el plugin."),
                ["fr"] = new PluginText(
                    "Organise depuis un seul endroit les onglets Steam, Decky et ceux de tes plugins préférés.",
                    @"Shortcuts réunit dans une interface pensée pour la manette les onglets natifs du menu d'accès rapide de Steam, Decky et les raccourcis de plugins compatibles. Les changements apparaissent immédiatement sur l'accueil de Steam comme en jeu.

## Ce qu'il fait
• Réorganise les onglets QAM natifs de Steam, Decky et les onglets personnalisés compatibles.
• Transforme les panneaux de plugins Decky chargés en raccourcis indépendants.
• Conserve l'entrée d'origine de chaque plugin dans Decky.
• Permet d'utiliser l'icône d'origine ou l'une des icônes Tabler incluses.
• Conserve les icônes choisies et l'ordre des onglets après un redémarrage de Steam ou de Shortcuts.
• Restaure les plugins temporairement indisponibles dès leur retour.

## Remarque
• Il utilise une intégration sécurisée avec Steam et le registre interne des onglets QAM de Decky, car aucune API publique ne gère actuellement les onglets principaux indépendants et leur ordre. Une future mise à jour de Steam ou de Decky pourra demander une adaptation du plugin."),
                ["de"] = new PluginText(
                    "Ordne Steam, Decky und deine bevorzugten Plugin-Tabs an einem Ort.",
                    @"Shortcuts bietet dir eine zentrale, controllerfreundliche Ansicht, in der du Steams native Schnellzugriffsmenü-Tabs, Decky und kompatible Plugin-Verknüpfungen anordnen kannst. Änderungen werden sofort auf der Steam-Startseite und im Spiel übernommen.

## Funktionen
• Ordnet Steams native QAM-Tabs, Decky und kompatible benutzerdefinierte Tabs neu an.
• Macht Bereiche geladener Decky-Plugins zu eigenständigen Verknüpfungen.
• Behält den ursprünglichen Eintrag jedes Plugins in Decky bei.
• Verwendet wahlweise das Originalsymbol oder eines der enthaltenen Tabler-Symbole.
• Speichert Symbolauswahl und Tab-Reihenfolge auch nach einem Neustart von Steam oder Shortcuts.
• Stellt vorübergehend nicht verfügbare Plugins wieder her, sobald sie zurückkehren.

## Hinweis
• Das Plugin nutzt eine abgesicherte Integration mit Steam und Deckys interne QAM-Tab-Verwaltung, da es derzeit keine öffentliche API für eigenständige Haupt-Tabs und deren Reihenfolge gibt. Ein künftiges Steam- oder Decky-Update kann eine Anpassung erfordern."),
                ["pt"] = new PluginText(
                    "Organiza num só lugar os separadores do Steam, do Decky e dos teus plugins favoritos.",
                    @"O Shortcuts reúne num único espaço pensado para comando os separadores nativos do menu de acesso rápido do Steam, o Decky e os atalhos de plugins compatíveis. As alterações aparecem de imediato tanto no ecrã inicial do Steam como durante o jogo.

## O que faz
• Reordena os separadores QAM nativos do Steam, o Decky e os separadores personalizados compatíveis.
• Transforma os painéis de plugins Decky carregados em atalhos independentes.
• Mantém disponível a entrada original de cada plugin no Decky.
• Permite usar o ícone original ou escolher entre os ícones Tabler incluídos.
• Guarda os ícones escolhidos e a ordem dos separadores após reiniciar o Steam ou o Shortcuts.
• Restaura os plugins temporariamente indisponíveis quando voltam.

## Nota
• Usa uma integração protegida com o Steam e o registo interno de separadores QAM do Decky, porque ainda não existe uma API pública que abranja separadores principais independentes e a respetiva ordem. Uma futura atualização do Steam ou do Decky poderá exigir uma adaptação do plugin."),
                ["uk"] = new PluginText(
                    "Упорядковуйте вкладки Steam, Decky й улюблених плагінів в одному місці.",
                    @"Shortcuts дає змогу в одному зручному для контролера інтерфейсі впорядковувати вбудовані вкладки меню швидкого доступу Steam, Decky та сумісні ярлики плагінів. Зміни відразу з'являються на головному екрані Steam і під час гри.

## Можливості
• Змінює порядок вбудованих вкладок QAM Steam, Decky та сумісних користувацьких вкладок.
• Перетворює панелі завантажених плагінів Decky на окремі ярлики.
• Зберігає початковий запис кожного плагіна в Decky.
• Дозволяє використовувати оригінальну іконку або одну з вбудованих іконок Tabler.
• Зберігає вибрані іконки та порядок вкладок після перезапуску Steam або Shortcuts.
• Відновлює тимчасово недоступні плагіни, коли вони знову з'являються.

## Примітка
• Плагін використовує захищену інтеграцію зі Steam і внутрішній реєстр вкладок QAM у Decky, оскільки публічного API для незалежних вкладок верхнього рівня та їх упорядкування поки немає. Майбутнє оновлення Steam або Decky може вимагати адаптації плагіна."),
                ["zh"] = new PluginText(
                    "在一个地方整理 Steam、Decky 和常用插件标签。",
                    @"Shortcuts 提供一个适合手柄操作的统一界面，用于排列 Steam 快捷菜单的原生标签、Decky 和兼容插件的快捷方式。更改会立即显示在 Steam 主界面和游戏中。

## 功能
• 调整 Steam 原生 QAM 标签、Decky 和兼容自定义标签的顺序。
• 将已加载 Decky 插件的面板变成独立快捷方式。
• 保留每个插件在 Decky 中的原始入口。
• 可使用插件原始图标，也可选择内置的 Tabler 图标。
• 在 Steam 或 Shortcuts 重启后保留图标选择和标签顺序。
• 暂时不可用的插件恢复后会自动重新显示。

## 注意
• 由于目前没有涵盖独立顶层标签及其排序的公开 API，本插件会以受保护的方式与 Steam 和 Decky 的内部 QAM 标签注册机制集成。未来的 Steam 或 Decky 更新可能需要相应调整插件。"),
                ["ja"] = new PluginText(
                    "Steam、Decky、お気に入りのプラグインタブを一か所で整理できます。",
                    @"Shortcuts は、Steam のクイックアクセスメニューにある標準タブ、Decky、対応プラグインのショートカットを、コントローラーで使いやすい一つの画面から並べ替えられます。変更は Steam のホーム画面でもゲーム中でもすぐに反映されます。

## 主な機能
• Steam の標準 QAM タブ、Decky、対応するカスタムタブを並べ替えます。
• 読み込み済み Decky プラグインのパネルを独立したショートカットにします。
• 各プラグインの Decky 内の元の項目もそのまま利用できます。
• 元のアイコン、または同梱の Tabler アイコンを選べます。
• Steam または Shortcuts の再起動後もアイコンとタブの順序を保持します。
• 一時的に利用できなかったプラグインが戻ると自動で復元します。

## 注意
• 独立したトップレベルタブとその順序を扱う公開 API がまだないため、Steam と Decky の内部 QAM タブレジストリに保護された方法で連携します。今後の Steam または Decky の更新に合わせてプラグインの調整が必要になる場合があります。"),
                ["ko"] = new PluginText(
                    "Steam, Decky, 즐겨 쓰는 플러그인 탭을 한곳에서 정리하세요.",
                    @"Shortcuts는 Steam의 기본 빠른 액세스 메뉴 탭, Decky, 호환 플러그인 바로가기를 컨트롤러에 맞춘 한 화면에서 정리할 수 있게 해 줍니다. 변경 사항은 Steam 홈 화면과 게임 중에 즉시 반영됩니다.

## 주요 기능
• Steam 기본 QAM 탭, Decky, 호환 사용자 지정 탭의 순서를 바꿉니다.
• 로드된 Decky 플러그인 패널을 독립 바로가기로 만듭니다.
• 각 플러그인의 원래 Decky 항목을 그대로 유지합니다.
• 원래 아이콘이나 포함된 Tabler 아이콘을 선택할 수 있습니다.
• Steam 또는 Shortcuts를 다시 시작해도 아이콘 선택과 탭 순서를 유지합니다.
• 일시적으로 사용할 수 없던 플러그인이 돌아오면 자동으로 복원합니다.

## 참고
• 독립적인 최상위 탭과 그 순서를 다루는 공개 API가 아직 없어 Steam 및 Decky의 내부 QAM 탭 레지스트리와 보호된 방식으로 연동합니다. 향후 Steam 또는 Decky 업데이트에 맞춰 플러그인 조정이 필요할 수 있습니다."),
                ["hi"] = new PluginText(
                    "Steam, Decky और अपने पसंदीदा प्लगइन टैब को एक ही जगह व्यवस्थित करें।",
                    @"Shortcuts आपको कंट्रोलर के अनुकूल एक ही जगह से Steam के मूल Quick Access Menu टैब, Decky और संगत प्लगइन शॉर्टकट व्यवस्थित करने देता है। बदलाव Steam Home और खेलते समय तुरंत दिखाई देते हैं।

## यह क्या करता है
• Steam के मूल QAM टैब, Decky और संगत कस्टम टैब का क्रम बदलता है।
• लोड किए गए Decky प्लगइन पैनल को स्वतंत्र शॉर्टकट में बदलता है।
• हर प्लगइन की मूल Decky प्रविष्टि उपलब्ध रखता है।
• मूल आइकन या शामिल Tabler आइकन में से किसी एक को चुनने देता है।
• Steam या Shortcuts के दोबारा शुरू होने के बाद भी आइकन और टैब का क्रम सुरक्षित रखता है।
• अस्थायी रूप से अनुपलब्ध प्लगइन वापस आने पर उन्हें पुनर्स्थापित करता है।

## नोट
• स्वतंत्र शीर्ष-स्तरीय टैब और उनका क्रम संभालने वाला कोई सार्वजनिक API अभी उपलब्ध नहीं है, इसलिए यह Steam और Decky की आंतरिक QAM टैब रजिस्ट्री के साथ सुरक्षित एकीकरण का उपयोग करता है। Steam या Decky के भविष्य के अपडेट के बाद प्लगइन में बदलाव की आवश्यकता हो सकती है।"),
                ["ru"] = new PluginText(
                    "Настраивайте вкладки Steam, Decky и любимых плагинов в одном месте.",
                    @"Shortcuts позволяет в одном удобном для контроллера интерфейсе упорядочивать встроенные вкладки меню быстрого доступа Steam, Decky и ярлыки совместимых плагинов. Изменения сразу появляются на главном экране Steam и во время игры.

## Возможности
• Меняет порядок встроенных вкладок QAM Steam, Decky и совместимых пользовательских вкладок.
• Превращает панели загруженных плагинов Decky в отдельные ярлыки.
• Сохраняет исходный пункт каждого плагина в Decky.
• Позволяет использовать исходный значок или выбрать один из встроенных значков Tabler.
• Сохраняет выбранные значки и порядок вкладок после перезапуска Steam или Shortcuts.
• Восстанавливает временно недоступные плагины, когда они снова появляются.

## Примечание
• Плагин использует защищённую интеграцию со Steam и внутренний реестр вкладок QAM в Decky, поскольку публичного API для независимых вкладок верхнего уровня и управления их порядком пока нет. Будущее обновление Steam или Decky может потребовать адаптации плагина.")
            },
